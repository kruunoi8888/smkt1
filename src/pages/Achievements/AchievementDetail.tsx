...
import { Achievement, SchoolConfig, MenuItem } from '@/types';
...
const AchievementDetail: React.FC<AchievementDetailViewProps> = ({ 
  achievementId, achievements, config, menus, isLoggedIn, onLogout, onView 
}) => {
    ...
    return (
      ...
      >
        ...
        <main className="flex-grow container mx-auto px-4 py-20 text-center">
          <h2 className="text-2xl font-bold text-slate-800 mb-4">ไม่พบข้อมูลรางวัลและผลงานที่คุณต้องการ</h2>
          <a href="#/" className="text-blue-600 font-bold hover:underline">← กลับไปหน้าแรก</a>
        </main>
        ...
        <div className="bg-white p-6 md:p-10 lg:p-12 rounded-[1.5rem] md:rounded-[2rem] border border-slate-100 shadow-sm max-w-5xl mx-auto">
          <div className="mb-8">
            ...
            <div className="flex items-center gap-2 mb-4">
              <span className="bg-yellow-100 text-yellow-700 text-[10px] md:text-xs font-black px-4 py-1.5 rounded-full uppercase tracking-widest shadow-sm">
                รางวัลและผลงาน {formatThaiDate(item.date)}
              </span>
            </div>
